...
mixer.init()
sound = mixer.Sound('ThienDangLive-WowyPhamDangAnhThu-6722192-_mp3cut.net_.wav')
...
def predict_eye_state(model, image):
    ...
    # Chuyen thanh tensor
    image_batch = np.reshape(image, (1, 128, 128, 1))
    # Dua vao mang mobilenet de xem mat dong hay mo
    image_batch = applications.mobilenet.preprocess_input(image_batch)
...
model = load_model('eye_detection_v1.h5')
...
while (True):
    ...
    if countClose > 1:
        cv2.putText(frame, "CANH BAO !!!!!!!", (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0),
